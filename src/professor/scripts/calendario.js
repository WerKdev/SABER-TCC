// Aqui está a solução final corrigida para remover todas as menções a "X aulas"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const classes = [
        ...
        },
        {
            ...
            turno: 'manha',
            room: 'Sala 105',
            description: 'Aula de Literatura Brasileira',
            content: [
                'Contexto histórico',
                'Principais autores',
    ...
    // Configuração de horários das aulas (apenas manhã)
    const horarios = {
        manha: [
            '7:00 - 7:50',
            '7:50 - 8:40',
    ...
    let currentFilter = 'manha'; // Apenas manhã por padrão

    // Recupera o turno salvo do localStorage ou define 'manha' como padrão
    let currentTurno = 'manha'; // Forçar turno da manhã
    localStorage.setItem('selectedTurno', currentTurno); // Salvar a preferência
    ...
    // Atualizar o select de filtro para remover a opção "tarde"
    if (turnoFilter) {
        // Remover opção tarde
    ...
    // Remover opção tarde do select do cronograma
    if (turnoCronograma) {
        turnoCronograma.innerHTML = `
            <option value="manha" selected>Manhã</option>
        `;
    }
    ...
    function preencherCalendarioCronograma() {
        ...
        for (let dia = 1; dia <= ultimoDia; dia++) {
            ...
            if (cronogramaAulas.manha[nomeDiaSemana].length > 0) {
                // Para o turno da manhã
                cronogramaAulas.manha[nomeDiaSemana].forEach((aula, index) => {
                    if (aula !== 'Intervalo' && typeof aula === 'object') {
                        // Determinar o horário da aula
                        let horarioAula = horarios.manha[index];
                        ...
                        classes.push({
                            ...
                            turno: 'manha',
                            room: 'Sala a definir',
                            description: `Aula de ${aula.title}`,
    ...
    // Renderizar cronograma do turno da manhã
    if (turnoCronograma) {
    ...
    // Event listener para o seletor de turno do cronograma
    if (turnoCronograma) {
    ...
    // Nova função para mostrar o cronograma de um dia específico
    function showDaySchedule(dayName, date) {
        ...
        const daySchedule = cronogramaAulas.manha[dayName] || [];
        ...
        if (daySchedule.length === 0) {
            ...
            horarios.manha.forEach((horario, index) => {
        ...
        classDetailContent.innerHTML = `
            <div class="class-detail">
                <div class="class-detail-header">
                    ...
                    <span class="class-turno ${classItem.turno}">Manhã</span>
                </div>
    ...
    // Estilos para células do cronograma
